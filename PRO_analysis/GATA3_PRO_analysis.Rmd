...
pre-alignment:
1) **Remove Adapters**: use `cutadapt` to remove provided adapter sequences, and also removes any adapter-adapter ligation sequences. \ 
...
We could also interpret the UMI size from the `cutadapt` report. There is a "Overview of removed sequences" table showing in the `cutadapt` output: the first and second column (length and count) will tell us X(length) bases were removed in Y (count) reads. It is expected that smaller bases are more likely to be found and removed, thus the corressponding count value will be large. However, in the table you would see that the second highest count number is found at a relatively high X. The UMI size should be the subtraction between read size and this X base value. \
