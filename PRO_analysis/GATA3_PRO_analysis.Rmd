...
The purpose of this analysis is to annotate gene properly and define primary transcripts 

## Data pre-process
In the directory with bigWig file of control libraries (no dTAG +/- E2), run the script below: \
```{r engine='bash', eval=F, echo=TRUE}
#! /bin/sh
...
# download the gencode.v44.basic.annotation.gtf.gz file from: https://www.gencodegenes.org/human/
# content: Basic gene annotation; region: CHR; Download: GTF


#mkdir Annotation_v44
#cd Annotation_v44
...
grep 'transcript_type "protein_coding"' gencode.v44.basic.annotation.gtf | \
    ...
    tr " " "\t" | tr -d '"' | \
    grep -v "\tENSG" > gencode.hg38.v44.basic.firstExon.latest.filtered.bed
...
#TSS_annot_input=read.table("/home/FCAM/ssun/Annotation_v44/gencode.hg38.v44.basic.firstExon.latest.filtered.bed",col.names=c('chrom', 'start', 'end', 'gene', 'v5', 'strand'))
gene.exon1 = parse.bed.exon1("/home/FCAM/ssun/Annotation_v44/gencode.hg38.v44.basic.firstExon.latest.filtered.bed")

# assign bigWig variables
# These two are the ones with control merged (no dTAG +/-E2)
...
save.image('231110_GATA3_pro_TSS.Rdata')
```
...
grep 'transcript_type "protein_coding"' gencode.v44.basic.annotation.gtf | \
   ...
   cut -f1,4,5,7,9 | tr ";" "\t" | \
   awk '{for(i=5;i<=NF;i++){if($i~/^gene_name/){a=$(i+1)}} print $1,$2,$3,a,"na",$4}' | \
   tr " " "\t" | tr -d '"' > gencode.v44.firstExon.bed
...
grep 'transcript_type "protein_coding"' gencode.v44.basic.annotation.gtf | \
    ...
    cut -f1,4,5,7,9 | tr ";" "\t" | \
    awk '{for(i=5;i<=NF;i++){if($i~/^gene_name/){a=$(i+1)}} print $1,$2,$3,a,"na",$4}' | \
    tr " " "\t" | tr -d '"' > gencode.v44.transcript.bed
```
...
fname = "/home/FCAM/ssun/Annotation_v44/gencode.v44.firstExon.bed"
...
fname = "/home/FCAM/ssun/Annotation_v44/gencode.v44.transcript.bed"
dat0 = read.table(fname,header=F,stringsAsFactors=F) 
names(dat0) = c('chr', 'start', 'end', 'gene', 'xy', 'strand') 
dat0 = unique(dat0)
...
```{r engine='R', eval=F, echo=TRUE}
RSAT_df <- function(file_path) {
  # Read the file
  data <- read.table(file_path, header = FALSE, sep = "\t")

  # Split the first column into components using strsplit
  components_list <- strsplit(data$V1, "n\\{|\\}")

  # Extract components from the list
  data$first <- sapply(components_list, function(x) x[1])  # "gat"
  data$Number <- as.numeric(sapply(components_list, function(x) x[2])) + 10  # Add 10 to the Number
  data$second <- sapply(components_list, function(x) x[3])  # "atc"
...
result <- extract_and_create_columns(file_path)
print(result)
...
#List of 2
# $ counts : Named num [1:19667] 0 0 0 198 123 ...
#  ..- attr(*, "names")= chr [1:19667] "OR4F5" "OR4F29" "OR4F16" "SAMD11" ...
# $ density: Named num [1:19667] 0 0 0 0.00959 0.00819 ...
#  ..- attr(*, "names")= chr [1:19667] "OR4F5" "OR4F29" "OR4F16" "SAMD11" ...
...
#57134 chr19 58345183 58353492    A1BG na      -
#31229 chr10 50799409 50885675    A1CF na      -
#37529 chr12  9067708  9115919     A2M na      -
#37520 chr12  8822621  8876787   A2ML1 na      +
#1410   chr1 33306766 33321098 A3GALT2 na      -
#60611 chr22 42692121 42720870  A4GALT na      -
...
#save.image('231110_GATA3_pro_PTA.Rdata')
...
#save.image('231110_GATA3_pro_PTA_final.Rdata')

# we have one chrM coordinates exceeds chrM end ((16809 > 16569): chrM	14746	16809	MT-CYB	0	+ 
...
```{r engine='R', eval=F, echo=TRUE}
inferred.coords=read.table('../PTA_ctrl/MCF7_GATA3_gene_annotations.bed', header =FALSE)
...
MCF7_dTAGGATAClone522_30min_control_E2_rep1 0.9507953 
MCF7_dTAGGATAClone522_30min_control_E2_rep2 0.9758427 
MCF7_dTAGGATAClone522_30min_control_E2_rep3  0.9425850 
MCF7_dTAGGATAClone522_30min_control_NoE2_rep1 0.9877159 
MCF7_dTAGGATAClone522_30min_control_NoE2_rep2 1.4483303 
MCF7_dTAGGATAClone522_30min_control_NoE2_rep3 0.9770686 
MCF7_dTAGGATAClone522_30min_dTAGV1_E2_rep1 0.9634926 
MCF7_dTAGGATAClone522_30min_dTAGV1_E2_rep2 0.9337290 
MCF7_dTAGGATAClone522_30min_dTAGV1_E2_rep3 0.9528725 
MCF7_dTAGGATAClone522_30min_dTAGV1_NoE2_rep1 1.0178932 
MCF7_dTAGGATAClone522_30min_dTAGV1_NoE2_rep2 0.9545367 
MCF7_dTAGGATAClone522_30min_dTAGV1_NoE2_rep3 0.9955123
...
out = matchit(treatment ~ baseMean, data = df.deseq.effects.lattice, method = "optimal", ratio = 1)
        ...
        #unchanged has 655 genes, activated has 891 genes. Change log2FoldChange cutoff for unchanged gene (from 0.01) to 0.015, which increase the number of unchanged genes from 638 to 951. Now we have enough unchanged to match with the activated genes.
...
# repressed
repressed = res1[res1$padj < 0.01 & !is.na(res1$padj) & res1$log2FoldChange < 0,] #727
...
```{r engine='R', eval=F, echo=TRUE}
save.image('231110_GATA3_pro_DEA_interaction1.Rdata')
```
...
```{r engine='R', eval=F, echo=TRUE}
save.image('231110_GATA3_pro_DEA_interaction1.1.Rdata')
```
...
# repressed: 28; activated: 283


ma.plot.lattice(ma.df, filename='FDR0.05_control_E2_vs_control_NoE2', title.main="Effect of E2 treatment (FDR <0.05)", col=c("grey90","grey60", "red", "blue"))
ma.plot.png.lattice(ma.df, filename='FDR0.05_control_E2_vs_control_NoE2', title.main="Effect of E2 treatment (FDR <0.05)", col=c("grey90","grey60", "red", "blue"))
```

```{r engine='R', eval=F, echo=TRUE}
save.image('231110_GATA3_pro_DEA_interaction1.2.Rdata')
```

```{r  out.width = "100%", echo=F, fig.align = "center", fig.cap="Effect of E2 treatment (FDR <0.05)"}
#library(knitr)
knitr::include_graphics("./png_image/MA_plot_FDR0.05_control_E2_vs_control_NoE2_response.png") 
```
...
############################### activated
activated = res_E2[res_E2$padj < 0.05 & !is.na(res_E2$padj) & res_E2$log2FoldChange > 0,] #283
activated$treatment = "Activated"

############################### repressed
repressed = res_E2[res_E2$padj < 0.05 & !is.na(res_E2$padj) & res_E2$log2FoldChange < 0,] #28
...
```{r  out.width = "100%", echo=F, fig.align = "center", fig.cap="ER-regulated genes upon E2 treatment (FDR <0.05)"}
#library(knitr)
knitr::include_graphics("./png_image/BW_plot_0.05_control_E2_vs_control_NoE2_response.png") 
```
...
```{r engine='R', eval=F, echo=TRUE}
ER_activated =as.data.frame(activated)[,c(2,7)] #283
res_dTAG_E2=as.data.frame(results(dds, contrast=c("supp.condition2","dTAG","ctrl"))) #12952

ER_activated_dTAG = res_dTAG_E2[rownames(res_dTAG_E2) %in% rownames(ER_activated),] #283
sum(ER_activated_dTAG$padj < 0.1 & !is.na(ER_activated_dTAG$padj)) # 191
sum(ER_activated_dTAG$padj < 0.05 & !is.na(ER_activated_dTAG$padj)) # 176
...
repressed= ER_activated_dTAG[ER_activated_dTAG$padj < 0.05 & !is.na(ER_activated_dTAG$padj) & ER_activated_dTAG$log2FoldChange < 0,] # 69

activated$treatment = "Activated*"
repressed$treatment = "Repressed*"

sig_ER_dTAG_df=as.data.frame(rbind(activated, repressed)) #176
```

1) ER-activated genes change expression status from active to repress upon GATA3 degradation

```{r engine='R', eval=F, echo=TRUE}
temp1=sig_ER_dTAG_df[sig_ER_dTAG_df$treatment=="Repressed*", ][,c(2,7)] #69
temp1$treatment="E2_plus_dTAG"
temp2=ER_activated[rownames(ER_activated) %in% rownames(temp1),] #69
...
```{r engine='R', eval=F, echo=TRUE}
dec_exp=change_exp[change_exp$treatment=="decrease_activation",] #35
...
```{r engine='R', eval=F, echo=F}
save.image('231110_bw.Rdata')
```

## [Coherence Check] Call the interaction term directly from DESeq2 results (play around)
...
sum(res1$padj < 0.1 & !is.na(res1$padj)) #1963
...
sum(res2.1$padj < 0.1 & !is.na(res2.1$padj)) #543
...
```{r engine='R', eval=F, echo=F}
save.image('231110_GATA3_pro_DEA_interaction2.Rdata')
```
...
## Questions: how GATA3 contributes to ER regulated genes? [ongoing]
...
We first use `samtools` to convert concordantly mapped reads (`-f 0x2`) and un-mapped(`-F 0X2`) reads to .fastq format, then use `fastqc` to check sequence quality (length distribution,...). \
...
I confirmed that in the raw fastq file, the two adapter sequences are indeed in the PE1 and PE2 reads before `cutadapt`, and trimmed after `cutadapt`. \
In normal situation, the sequences from raw fastq will have features of: PE1 reads with 5' adapter, PE2 reads with 3' adapter;
However, I found some unexpected cases where **some PE1 sequences somehow has reverse compliment of 3' adapter**, and **some PE2 sequences has reversed 5' adapter**. `cutadapt` will not trim these off and these sequences are carried on to next steps. \
...
#cat MCF7_dTAGGATAClone522_30min_control_E2_rep1_PE1_final_test.fastq | grep CCTTGGCACCCGAGAATTCCA | wc -l #0. rv 5' adp
#cat MCF7_dTAGGATAClone522_30min_control_E2_rep1_PE1_final_test.fastq | grep GATCGTCGGACTGTAGAACTCTGAAC | wc -l #0 3' adp
#cat MCF7_dTAGGATAClone522_30min_control_E2_rep1_PE1_final_test.fastq | grep GTTCAGAGTTCTACAGTCCGACGATC | wc -l #143 rv 3' adp
...
#cat MCF7_dTAGGATAClone522_30min_control_E2_rep1_PE2_final_test.fastq | grep GTTCAGAGTTCTACAGTCCGACGATC | wc -l #0 rv 3' adp
#cat MCF7_dTAGGATAClone522_30min_control_E2_rep1_PE2_final_test.fastq | grep TGGAATTCTCGGGTGCCAAGG | wc -l #0 5' adp
#cat MCF7_dTAGGATAClone522_30min_control_E2_rep1_PE2_final_test.fastq | grep CCTTGGCACCCGAGAATTCCA | wc -l #73980. rv 5' adp
...
#cat MCF7_dTAGGATAClone522_30min_control_E2_rep1_PE1_test_noadap.fastq | grep GTTCAGAGTTCTACAGTCCGACGATC | wc -l #86 rv 3' adp
cat MCF7_dTAGGATAClone522_30min_control_E2_rep1_PE2_test_noadap.fastq | grep GATCGTCGGACTGTAGAACTCTGAAC | wc -l
#cat MCF7_dTAGGATAClone522_30min_control_E2_rep1_PE2_test_noadap.fastq | grep CCTTGGCACCCGAGAATTCCA | wc -l #73979 rv 5' adp
```

## two-step cutadapt try out
```{r engine='R', eval=F, echo=TRUE}
file_PE1=/labs/Guertin/siyu/Sathyan_GATA3_PRO_final/final_raw/MCF7_dTAGGATAClone522_30min_control_E2_rep1_PE1_final.fastq
file_PE2=/labs/Guertin/siyu/Sathyan_GATA3_PRO_final/final_raw/MCF7_dTAGGATAClone522_30min_control_E2_rep1_PE2_final.fastq
# 1st cutadapt: we have used  -m $((UMI_length+2)) and $((UMI_length+10)) to filter any sequences that are shorter than 10; -O sets the minimum overlap parameters 
module load cutadapt/3.5
cutadapt -m 10 -O 1 -a TGGAATTCTCGGGTGCCAAGG $file_PE1 -o PE1_test_noadap1.fastq --too-short-output test_PE1_short1.fastq > PE1_test_cutadapt1.txt
cutadapt -m 18 -O 1 -a GATCGTCGGACTGTAGAACTCTGAAC $file_PE2 -o PE2_test_noadap1.fastq --too-short-output test_PE2_short1.fastq > PE2_test_cutadapt1.txt

# 2nd cutadapt
cutadapt -m 10 -O 10 -a TGGAATTCTCGGGTGCCAAGG PE1_test_noadap1.fastq -o PE1_test_noadap2.fastq --too-short-output test_PE1_short2.fastq > PE1_test_cutadapt2.txt
cutadapt -m 18 -O 10 -a GATCGTCGGACTGTAGAACTCTGAAC PE2_test_noadap1.fastq -o PE2_test_noadap2.fastq --too-short-output test_PE2_short2.fastq > PE2_test_cutadapt2.txt


wc -l PE1_test_noadap1.fastq | awk '{print $1/4}' #29473968
wc -l PE2_test_noadap1.fastq | awk '{print $1/4}' #33116654
wc -l PE1_test_noadap2.fastq | awk '{print $1/4}' #28845586
wc -l PE2_test_noadap2.fastq | awk '{print $1/4}' #33115759

cutadapt -m 10 -O 10 --rc -a TGGAATTCTCGGGTGCCAAGG PE1_test_noadap1.fastq -o PE1_test_noadap3.fastq --too-short-output test_PE1_short3.fastq > PE1_test_cutadapt3.txt
cutadapt -m 18 -O 10 --rc -a GATCGTCGGACTGTAGAACTCTGAAC PE2_test_noadap1.fastq -o PE2_test_noadap3.fastq --too-short-output test_PE2_short3.fastq > PE2_test_cutadapt3.txt

wc -l PE1_test_noadap3.fastq | awk '{print $1/4}' #28845563
wc -l PE2_test_noadap3.fastq | awk '{print $1/4}' #33115713

for i in *noadap2.fastq
do
 fastqc $i
 echo $i
 echo "fastqc complete"
...
#SBATCH --job-name=pro_processing_two_cutadapt.sh     
#SBATCH -N 1                  
#SBATCH -n 1                 
#SBATCH -c 32                  
#SBATCH -p himem           
...
#SBATCH --mail-type=ALL 
#SBATCH --mail-user=ssun@uchc.edu
#SBATCH -o pro_processing_two_cutadapt.sh_%j.out
#SBATCH -e pro_processing_two_cutadapt.sh_%j.err

hostname
set -e
export PATH=$PATH:/home/FCAM/ssun/packages/://home/FCAM/ssun/scripts
#directory=/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/Pro_final/final_processed/unmap/cutadapt/
...
#echo 'removing dual adapter ligations and calculating the fraction of adapter/adapters in' $name
echo 'removing short RNA insertions' 
seqtk seq -L $((UMI_length+10)) PE1_test_noadap2.fastq > PE1_test_noadap2_trimmed.fastq # removes inserts that are shorter than 10 bases

echo 'removing PCR duplicates'
fqdedup -i PE1_test_noadap2_trimmed.fastq -o PE1_test_dedup.fastq #remove PCR duplicates
PE1_noAdapter=$(wc -l PE1_test_dedup.fastq | awk '{print $1/4}') #this variable is a near-optimal table size value for fastq_pair
fastq_pair -t $PE1_noAdapter PE1_test_dedup.fastq PE2_test_noadap2.fastq #pair FASTQ files


echo 'calculating and plotting RNA insert sizes'
# measure RNA degradation by searching for overlap between paired end reads with flash and plotting the resultant histogram output with insert_size.R
flash -q --compress-prog=gzip --suffix=gz PE1_test_dedup.fastq.paired.fq \
        PE2_test_noadap2.fastq.paired.fq -o test
insert_size.R test.hist ${UMI_length}


echo 'trimming off the UMI'
# removes the UMI and reverse complements from paired end reads
seqtk trimfq -b ${UMI_length} PE1_test_dedup.fastq | seqtk seq -r - > PE1_processed.fastq
seqtk trimfq -e ${UMI_length} PE2_test_noadap2.fastq | seqtk seq -r - > PE2_processed.fastq


echo 'aligning to rDNA and removing aligned reads'
    bowtie2 -p $((cores-2)) -x $prealign_rdna_index -U PE1_processed.fastq 2>test_bowtie2_rDNA.log | \
        samtools sort -n - | samtools fastq -f 0x4 - > PE1.rDNA.fastq
# removes PE2-aligned reads with a rDNA-aligned mate
reads=$(wc -l PE1.rDNA.fastq | awk '{print $1/4}')
fastq_pair -t $reads PE1.rDNA.fastq PE2_processed.fastq


echo 'aligning to the genome'
bowtie2 -p $((cores-2)) --maxins 1000 -x $genome_index --rf -1 PE1.rDNA.fastq.paired.fq \
        -2 PE2_processed.fastq.paired.fq 2>test_bowtie2.log | samtools view -b - | \
        samtools sort - -o test.bam # align to the genome;  --rf flag is to account for the fact that we reverse complemented both reads; samtools convert the file to BAM and sort the reads. 
        

#calculate the total number of rDNA-aligned reads
PE1_prior_rDNA=$(wc -l PE1_processed.fastq | awk '{print $1/4}')
PE1_post_rDNA=$(wc -l PE1.rDNA.fastq | awk '{print $1/4}')
total_rDNA=$(echo "$(($PE1_prior_rDNA-$PE1_post_rDNA))") 
#calculate the total that concordantly align to hg38 and/or rDNA
concordant_pe1=$(samtools view -c -f 0x42 test.bam)
total=$(echo "$(($concordant_pe1+$total_rDNA))")
#rDNA alignment rate
rDNA_alignment=$(echo "scale=2 ; $total_rDNA / $total" | bc)
echo -e "$rDNA_alignment\t$name\t0.10\trDNA Alignment Rate" >> QC_metrics.txt

#Mappability rate (expect an alignment rate above 80% for high quality libraries)
map_pe1=$(samtools view -c -f 0x42 test.bam)
pre_alignment=$(wc -l PE1.rDNA.fastq.paired.fq | awk '{print $1/4}')
alignment_rate=$(echo "scale=2 ; $map_pe1 / $pre_alignment" | bc)
echo -e "$alignment_rate\t$name\t0.80\tAlignment Rate" >> QC_metrics.txt
...
#test.bam
module load samtools/1.16.1
samtools fastq -F 0x82 test.bam > test_unmapped_PE1.fastq
samtools fastq -F 0x42 test.bam > test_unmapped_PE2.fastq
samtools fastq -f 0x82 test.bam > test_mapped_PE2.fastq
samtools fastq -f 0x42 test.bam > test_mapped_PE1.fastq
...
fastq results shows that, by perform a second cutadapt with minumum overlap of 10bp, we no longer see overrepresented 5' adapters in mapped reads; but the PE2 unmapped reads still contains some 21bp 5'adapter. I suspect these adapter reads are come from the unexpected case where raw fastq PE2 reads has rc of 5' adapter.

<!--
```{r engine='R', eval=F, echo=TRUE}
#manually remove reversed opposite adapter
cutadapt -m 10 -O 15 -a GTTCAGAGTTCTACAGTCCGACGATC PE1_test_noadap1.fastq -o PE1_test_noadap4.fastq --too-short-output test_PE1_short4.fastq > PE1_test_cutadapt4.txt #0.1% reads with reversed 3' adapter at PE1
cutadapt -m 18 -O 15 -a CCTTGGCACCCGAGAATTCCA PE2_test_noadap1.fastq -o PE2_test_noadap4.fastq --too-short-output test_PE2_short4.fastq > PE2_test_cutadapt4.txt #3.3% reads with reversed 5' adapter at PE2
...
#echo 'removing dual adapter ligations and calculating the fraction of adapter/adapters in' $name
echo 'removing short RNA insertions' 
seqtk seq -L $((UMI_length+10)) PE1_test_noadap4.fastq > PE1_test_noadap4_trimmed.fastq # removes inserts that are shorter than 10 bases

echo 'removing PCR duplicates'
fqdedup -i PE1_test_noadap4_trimmed.fastq -o PE1_test_dedup.fastq #remove PCR duplicates
PE1_noAdapter=$(wc -l PE1_test_dedup.fastq | awk '{print $1/4}') #this variable is a near-optimal table size value for fastq_pair
fastq_pair -t $PE1_noAdapter PE1_test_dedup.fastq PE2_test_noadap4.fastq #pair FASTQ files


echo 'calculating and plotting RNA insert sizes'
# measure RNA degradation by searching for overlap between paired end reads with flash and plotting the resultant histogram output with insert_size.R
flash -q --compress-prog=gzip --suffix=gz PE1_test_dedup.fastq.paired.fq \
        PE2_test_noadap4.fastq.paired.fq -o test
insert_size.R test.hist ${UMI_length}
...
# removes the UMI and reverse complements from paired end reads
seqtk trimfq -b ${UMI_length} PE1_test_dedup.fastq | seqtk seq -r - > PE1_processed.fastq
seqtk trimfq -e ${UMI_length} PE2_test_noadap4.fastq | seqtk seq -r - > PE2_processed.fastq
...
#set5
#manually remove reversed opposite adapter
cutadapt -m 10 -O 15 -a GTTCAGAGTTCTACAGTCCGACGATC ../PE1_test_noadap2.fastq -o PE1_test_noadap5.fastq --too-short-output test_PE1_short5.fastq > PE1_test_cutadapt5.txt #0.1% reads with reversed 3' adapter at PE1
cutadapt -m 18 -O 15 -a CCTTGGCACCCGAGAATTCCA ../PE2_test_noadap2.fastq -o PE2_test_noadap5.fastq --too-short-output test_PE2_short5.fastq > PE2_test_cutadapt5.txt #3.3% reads with reversed 5' adapter at PE2
...
#echo 'removing dual adapter ligations and calculating the fraction of adapter/adapters in' $name
echo 'removing short RNA insertions' 
seqtk seq -L $((UMI_length+10)) PE1_test_noadap5.fastq > PE1_test_noadap5_trimmed.fastq # removes inserts that are shorter than 10 bases

echo 'removing PCR duplicates'
fqdedup -i PE1_test_noadap5_trimmed.fastq -o PE1_test_dedup.fastq #remove PCR duplicates
PE1_noAdapter=$(wc -l PE1_test_dedup.fastq | awk '{print $1/4}') #this variable is a near-optimal table size value for fastq_pair
fastq_pair -t $PE1_noAdapter PE1_test_dedup.fastq PE2_test_noadap5.fastq #pair FASTQ files


echo 'calculating and plotting RNA insert sizes'
# measure RNA degradation by searching for overlap between paired end reads with flash and plotting the resultant histogram output with insert_size.R
flash -q --compress-prog=gzip --suffix=gz PE1_test_dedup.fastq.paired.fq \
        PE2_test_noadap5.fastq.paired.fq -o test
insert_size.R test.hist ${UMI_length}
...
# removes the UMI and reverse complements from paired end reads
seqtk trimfq -b ${UMI_length} PE1_test_dedup.fastq | seqtk seq -r - > PE1_processed.fastq
seqtk trimfq -e ${UMI_length} PE2_test_noadap5.fastq | seqtk seq -r - > PE2_processed.fastq
...
fastq results shows that, by perform a second cutadapt, then trimming the revrse complement adapter sequences with minumum overlap of 10bp, we no longer see overrepresented 5' adapters in the final reads (mapped and unmapped). \

There are overrepresented RNA primer index, also trim these \
```{r engine='R', eval=F, echo=TRUE}
#set6

module load cutadapt/3.5
# manually remove RPI adapter(reversed)
cutadapt -m 10 -O 18 -a CTCGGGTGCCAAGGAACTCCAGTCAC ../set5/PE1_test_noadap5.fastq -o PE1_test_noadap6.fastq --too-short-output test_PE1_short6.fastq > PE1_test_cutadapt6.txt #0.3% reads with RPI adapter(reversed)

# align to genome

#echo 'removing dual adapter ligations and calculating the fraction of adapter/adapters in' $name
echo 'removing short RNA insertions' 
seqtk seq -L $((UMI_length+10)) PE1_test_noadap6.fastq > PE1_test_noadap6_trimmed.fastq # removes inserts that are shorter than 10 bases

echo 'removing PCR duplicates'
fqdedup -i PE1_test_noadap6_trimmed.fastq -o PE1_test_dedup.fastq #remove PCR duplicates
PE1_noAdapter=$(wc -l PE1_test_dedup.fastq | awk '{print $1/4}') #this variable is a near-optimal table size value for fastq_pair
fastq_pair -t $PE1_noAdapter PE1_test_dedup.fastq ../set5/PE2_test_noadap5.fastq #pair FASTQ files


echo 'calculating and plotting RNA insert sizes'
# measure RNA degradation by searching for overlap between paired end reads with flash and plotting the resultant histogram output with insert_size.R
flash -q --compress-prog=gzip --suffix=gz PE1_test_dedup.fastq.paired.fq \
        ../set5/PE2_test_noadap5.fastq.paired.fq -o test
insert_size.R test.hist ${UMI_length}
...
# removes the UMI and reverse complements from paired end reads
seqtk trimfq -b ${UMI_length} PE1_test_dedup.fastq | seqtk seq -r - > PE1_processed.fastq
seqtk trimfq -e ${UMI_length} ../set5/PE2_test_noadap5.fastq | seqtk seq -r - > PE2_processed.fastq
...
```{r engine='R', eval=F, echo=TRUE}
module load samtools/1.16.1
map_pe2=$(samtools view -c -f 0x82 test.bam)
pre_alignment=$(wc -l PE2_processed.fastq.paired.fq | awk '{print $1/4}')
alignment_rate=$(echo "scale=2 ; $map_pe2 / $pre_alignment" | bc)

#aligning PE2 gives same 74% alignment rate --expected
```

**unmapped reads**
```{r engine='R', eval=F, echo=TRUE}
20162145 reads; of these:
  20162145 (100.00%) were paired; of these:
    5070062 (25.15%) aligned concordantly 0 times
    9970755 (49.45%) aligned concordantly exactly 1 time
    5121328 (25.40%) aligned concordantly >1 times
    ----
    5070062 pairs aligned concordantly 0 times; of these:
      264187 (5.21%) aligned discordantly 1 time
    ----
    4805875 pairs aligned 0 times concordantly or discordantly; of these:
      9611750 mates make up the pairs; of these:
        4883292 (50.81%) aligned 0 times
        2490603 (25.91%) aligned exactly 1 time
        2237855 (23.28%) aligned >1 times
87.89% overall alignment rate

#set6
module load samtools/1.16.1
samtools view -c test.bam  #40324290
samtools view -b -F 2 test.bam > test_nca.bam

# 1) discordantly aligned as pairs (994847*2)
samtools view -c -f 161 -F 28 test_nca.bam #PE2                #203679   
samtools view -c -f 81 -F 44 test_nca.bam #PE1 reverse strand   

samtools view -c -f 145  -F 44 test_nca.bam #PE2 reverse strand. #427148  
samtools view -c -f 97  -F 28 test_nca.bam #PE1                  

samtools view -c -f 177 -F 12 test_nca.bam #PE2 reverse strand   #364020 
samtools view -c -f 113 -F 12 test_nca.bam #PE1 reverse strand

# 2) read 1 in one locus & read2 in completely different locus (153520*2) --split reads
samtools view -c -f 129 -F 60 test_nca.bam #PE2.        #153520
samtools view -c -f 65 -F 60 test_nca.bam #PE1.         


# 3) one mate aligned, the other mate not aligned (2960098*2)
samtools view -c -f 165 -F 24 test_nca.bam #PE2 un-map               #1230983 
samtools view -c -f 89 -F 36 test_nca.bam #PE1 reverse but map.      

samtools view -c -f 153 -F 36 test_nca.bam #PE2 reverse but map.    # 308753
samtools view -c -f 101 -F 24 test_nca.bam #PE1 un-map              

samtools view -c -f 137 -F 52 test_nca.bam #PE2 map non-reverse           #157420
samtools view -c -f 69 -F 56 test_nca.bam #PE1 un-map.                    

samtools view -c -f 133 -F 56 test_nca.bam #PE2 un-map              # 1262942
samtools view -c -f 73 -F 52 test_nca.bam #PE1. map non-reverse     


# 4) not align at all
samtools view -c -f 141 -F 48 test_nca.bam #PE2 un-map            #961597
samtools view -c -f 77 -F 48 test_nca.bam #PE1 un-map            
...
library("lattice") 
bar=read.csv('MCF7_4adapt_unmapped_reads_bar.csv')
...
pdf('bar_MCF7_dTAGGATAClone522_30min_control_E2_rep1_unmapped.pdf', width=5,height=6)
print(barchart(fraction ~ supp,         
         data = bar,
...
bar=read.csv('compare_unmapped_reads_bar.csv')
bar$supp<- factor(bar$supp, levels = c("concordantly_aligned", "discordant_orientation", "split_reads"          , "only_one_mate_align", "not_align_at_all"))

pdf('bar_compare_unmapped.pdf', width=8,height=6)
print(barchart(fraction ~ supp,         
         data = bar,
         groups = lib,
         #stack = TRUE,
...
```{r engine='R', eval=F, echo=TRUE}
#set6
# PE1 unmap, but PE2 map (test_setA)
samtools fastq -f 153 -F 36 test_nca.bam > PE2_test1.fastq #PE2 reverse but map.
samtools fastq -f 101 -F 24 test_nca.bam > PE1_test1.fastq #PE1 un-map  

samtools fastq -f 137 -F 52 test_nca.bam > PE2_test2.fastq #PE2 map non-reverse    
samtools fastq -f 69 -F 56 test_nca.bam > PE1_test2.fastq #PE1 un-map.   

cat PE1_test1.fastq PE1_test2.fastq > test_setA_unmap_PE1.fastq
cat PE2_test1.fastq PE2_test2.fastq > test_setA_map_PE2.fastq

# PE1 map, but PE2 unmap (test_setB)
samtools fastq -f 165 -F 24 test_nca.bam > PE2_test3.fastq #PE2 un-map              
samtools fastq -f 89 -F 36 test_nca.bam > PE1_test3.fastq #PE1 reverse but map. 

samtools fastq -f 133 -F 56 test_nca.bam > PE2_test4.fastq #PE2 un-map               
samtools fastq -f 73 -F 52 test_nca.bam > PE1_test4.fastq #PE1. map non-reverse

cat PE1_test3.fastq PE1_test4.fastq > test_setB_map_PE1.fastq
cat PE2_test3.fastq PE2_test4.fastq > test_setB_unmap_PE2.fastq

# PE1 unmap, and PE2 unmap (test_setC)
samtools fastq -f 141 -F 48 test_nca.bam > test_setC_unmap_PE2.fastq #PE2 un-map            
samtools fastq -f 77 -F 48 test_nca.bam > test_setC_unmap_PE1.fastq #PE1 un-map
...
do
 ...
 sed -n '1~4s/^@/>/p;2~4p' $i > ${name}.fasta
 #get read length
 cat ${name}.fasta | awk '$0 ~ ">" {print c;c=0} $0 !~ ">" {c+=length($0);} END { print c; }' | sed '/^$/d' | sort | uniq -c | awk '{print $1,$2}' > ${name}_read_length.txt
done
```
...
## BLAST to nt
**BLAST on web**
To run BLAST on NCBI web, we need to first divide large sequence file (.fasta) to smaller sequence file due to web's size limitation on input files.
```{r engine='R', eval=F, echo=TRUE}
...
**Blast un-mapped reads locally** \
1) Blast to full nt database \
To save running time, first we need to divide the large .fa sequence file to smaller file contains 500 sequences (i.e., test2-500.00001); running multiple files at the same time give us 2k to 10k random sequence blast output to make some conclusions. \
...
## Why many "unmapped reads" can BLAST to human genome?
SAM Flags can extract reads with different features (reference: https://broadinstitute.github.io/picard/explain-flags.html): \
**Concordantly aligned reads**: (-f 0x2) R1 (forward)——-> expected mapping distance <——- R2 (reverse) \
note that in our pipeline, we have reversed R1 and R2, and we set the maximum mapping distance between them as 1000bp (--maxins 1000).\ 
**discordantly aligned reads**: these reads can align to human genome, but with 1) unexpected mapping distance; 2) unexpected read orientation; or 3) read 1/2 align to completely different locus. \ 
**not aligned reads**: 1) reads with one mate aligned, the other not; 2) both mate not align to genome. \
...
1) both reads aligned, but read1 or read2 or both reads has unexpected orientation; \
2) both reads aligned, but read 1 align to one locus & read2 align to completely different locus; \
3) one mate aligned, the other mate not aligned; \
4) both reads not align at all . \
...
Check if SAM Flag called unmapped reads are indeed not align to genome (bowtie2). \
```{r engine='R', eval=F, echo=TRUE}
module load samtools/1.16.1
...
### BLAST
blastn for "reads that not aligned at all" (e-value 0.001)
```{r engine='R', eval=F, echo=TRUE}
#module load samtools/1.16.1
#dir=/labs/Guertin/siyu/Sathyan_GATA3_PRO_final/final_processed/bam/Analysis/
...
# module load samtools/1.16.1
# samtools view -b -F 2 /labs/Guertin/siyu/HEK_CloneZD29_30min_control_rep2.bam >HEK_CloneZD29_30min_control_rep2_nca.bam
# PE1 unmap, but PE2 map (HEK_setA)
samtools fastq -f 153 -F 36 HEK_CloneZD29_30min_control_rep2_nca.bam > PE2_test1.fastq #PE2 reverse but map.# 489522
samtools fastq -f 101 -F 24 HEK_CloneZD29_30min_control_rep2_nca.bam > PE1_test1.fastq #PE1 un-map  # 489522

samtools fastq -f 137 -F 52 HEK_CloneZD29_30min_control_rep2_nca.bam > PE2_test2.fastq #PE2 map non-reverse     #207603
samtools fastq -f 69 -F 56 HEK_CloneZD29_30min_control_rep2_nca.bam > PE1_test2.fastq #PE1 un-map.   

cat PE1_test1.fastq PE1_test2.fastq > HEK_setA_unmap_PE1.fastq
cat PE2_test1.fastq PE2_test2.fastq > HEK_setA_map_PE2.fastq

# PE1 map, but PE2 unmap (HEK_setB)
samtools fastq -f 165 -F 24 HEK_CloneZD29_30min_control_rep2_nca.bam > PE2_test3.fastq #PE2 un-map               # 1424033
samtools fastq -f 89 -F 36 HEK_CloneZD29_30min_control_rep2_nca.bam > PE1_test3.fastq #PE1 reverse but map. 

samtools fastq -f 133 -F 56 HEK_CloneZD29_30min_control_rep2_nca.bam > PE2_test4.fastq #PE2 un-map              #1495474     
samtools fastq -f 73 -F 52 HEK_CloneZD29_30min_control_rep2_nca.bam > PE1_test4.fastq #PE1. map non-reverse

cat PE1_test3.fastq PE1_test4.fastq > HEK_setB_map_PE1.fastq
cat PE2_test3.fastq PE2_test4.fastq > HEK_setB_unmap_PE2.fastq
...
awk '/eudicots/' RS="Tax BLAST report" ORS="" MCF_setC_unmap_PE1_2k_OR.txt > blastn_MCF7_2k_OR_eudicots.txt
cat blastn_MCF7_2k_OR_eudicots.txt | grep "Query=" | wc -l #123
...
awk '/eudicots/' RS="Tax BLAST report" ORS="" HEK_setC_unmap_PE1_2k_OR.txt > blastn_HEK_2k_OR_eudicots.txt
cat blastn_HEK_2k_OR_eudicots.txt | grep "Query=" | wc -l #92
```
...
library("lattice") 
bar4=read.csv('blastn_setc.csv')
bar4$species_hit<- factor(bar4$species_hit, levels = c("no_hits", "eukaryotes", "phage", "bacteria", "fish", "moth", "eudicots"))

pdf('bar_blastn_setc_unmapped_MCF_vs_HEK.pdf', width=8,height=5)
print(barchart(percentage ~ species_hit,         
         data = bar4,
         groups = supp,
         #stack = TRUE,
         auto.key=list(space="right"),
...
### Align unmapped reads to CAS9/DNA constructs with bowtie2 
```{r engine='R', eval=F, echo=TRUE}
...
```{r engine='R', eval=F, echo=TRUE}
cat T47D_DMSO_rep1_bowtie2.log
13576060 reads; of these:
  13576060 (100.00%) were paired; of these:
    2008218 (14.79%) aligned concordantly 0 times
    7976629 (58.76%) aligned concordantly exactly 1 time
    3591213 (26.45%) aligned concordantly >1 times
    ----
    2008218 pairs aligned concordantly 0 times; of these:
      158008 (7.87%) aligned discordantly 1 time
    ----
    1850210 pairs aligned 0 times concordantly or discordantly; of these:
      3700420 mates make up the pairs; of these:
        2069978 (55.94%) aligned 0 times
        908380 (24.55%) aligned exactly 1 time
        722062 (19.51%) aligned >1 times
92.38% overall alignment rate


# 1) discordantly aligned as pairs (456800*2)
samtools view -c -f 161 -F 28 T47D_DMSO_rep1_nca.bam #PE2                #101352     
samtools view -c -f 81 -F 44 T47D_DMSO_rep1_nca.bam #PE1 reverse strand   

samtools view -c -f 145  -F 44 T47D_DMSO_rep1_nca.bam #PE2 reverse strand. #222274 
samtools view -c -f 97  -F 28 T47D_DMSO_rep1_nca.bam #PE1                  

samtools view -c -f 177 -F 12 T47D_DMSO_rep1_nca.bam #PE2 reverse strand   # 133174
samtools view -c -f 113 -F 12 T47D_DMSO_rep1_nca.bam #PE1 reverse strand

# 2) read 1 in one locus & read2 in completely different locus (15119*2) --split reads
samtools view -c -f 129 -F 60 T47D_DMSO_rep1_nca.bam #PE2.        #15119
samtools view -c -f 65 -F 60 T47D_DMSO_rep1_nca.bam #PE1.         


# 3) one mate aligned, the other mate not aligned (1002620*2)
samtools view -c -f 165 -F 24 T47D_DMSO_rep1_nca.bam #PE2 un-map               # 394626
samtools view -c -f 89 -F 36 T47D_DMSO_rep1_nca.bam #PE1 reverse but map.      

samtools view -c -f 153 -F 36 T47D_DMSO_rep1_nca.bam #PE2 reverse but map.    # 128224
samtools view -c -f 101 -F 24 T47D_DMSO_rep1_nca.bam #PE1 un-map              

samtools view -c -f 137 -F 52 T47D_DMSO_rep1_nca.bam #PE2 map non-reverse           #73992
samtools view -c -f 69 -F 56 T47D_DMSO_rep1_nca.bam #PE1 un-map.                    

samtools view -c -f 133 -F 56 T47D_DMSO_rep1_nca.bam #PE2 un-map              # 405778    
samtools view -c -f 73 -F 52 T47D_DMSO_rep1_nca.bam #PE1. map non-reverse     


# 4) not align at all
samtools view -c -f 141 -F 48 T47D_DMSO_rep1_nca.bam #PE2 un-map            # 533679
samtools view -c -f 77 -F 48 T47D_DMSO_rep1_nca.bam #PE1 un-map       
```
...
library("lattice") 
bar3=read.csv('T47D_unmapped_reads_bar.csv')
bar3$supp<- factor(bar3$supp, levels = c("concordantly_aligned", "discordant_orientation", "split_reads"          , "only_one_mate_align", "not_align_at_all"))

pdf('bar_T47D_DMSO_rep1_unmapped.pdf', width=5,height=6)
print(barchart(percentage ~ supp,         
         data = bar3,
         #groups = enriched_motif,
         #stack = TRUE,
