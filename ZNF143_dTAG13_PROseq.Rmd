---
title: \sf ZNF143 degron PRO-seq Analysis
header-includes:
- \usepackage{color}
- \usepackage{float}
...
```
Note: seqOutBias requires loading genometools/v1.5.10 
...
**IMPORTANT**: If you're planning on swarming the cluster for this next step, first \
run seqOutBias on just one (1) file. This generates the necessary intermediate files\
for your particular reference genome, a long and crucial step that only needs to be \
completed once. If you have already run seqOutBias before, make sure your working \
directory contains these intermediate files, or use "--gt-workdir=<path>" in the \
seqOutBias step to specify the directory where those files are located. 

After running seqOutBias for the first time, you are free to swarm the rest of your\
files as you like.

```
#! /usr/bin/bash

#SBATCH --job-name=seqOutBias_only_HEK_ZNF143FKBP_DMSO_rep3
#SBATCH -N 1                    
#SBATCH -n 1                  
#SBATCH -c 2                          
#SBATCH -p general
...
genome=/home/FCAM/jdong/human38/hg38.fa
readsize=38
filename=HEK_ZNF143FKBP_DMSO_rep3_PE1.fastq

name=$(echo $filename | awk -F "_PE1.fastq" '{print $1}')
echo "Current sample: ${name}"
